# 🚀 HD App - Production Deployment Script (PowerShell)
...
Write-Host "🚀 HD App - Production Deployment wird gestartet..." -ForegroundColor Blue
...
# Schritt 1: Prüfe ob env.production existiert
if (-not (Test-Path "env.production")) {
    Write-Error "env.production Datei nicht gefunden!"
    Write-Info "Bitte erstellen Sie eine env.production Datei basierend auf env.production.template"
    exit 1
}
...
"@
        ...
        $nginxConfig | Out-File -FilePath "nginx-hd-app.conf" -Encoding UTF8
        Write-Success "Nginx-Konfiguration erstellt: nginx-hd-app.conf"
...
Write-Host ""
Write-Success "🎉 HD App Production Deployment abgeschlossen!"
Write-Host ""
...
Write-Host "=====================" -ForegroundColor White
Write-Host "Logs anzeigen:    docker-compose -f docker-compose.prod.yml logs -f" -ForegroundColor Yellow
Write-Host "Services stoppen: docker-compose -f docker-compose.prod.yml down" -ForegroundColor Yellow
Write-Host "Services starten: docker-compose -f docker-compose.prod.yml up -d" -ForegroundColor Yellow
Write-Host "Status prüfen:    docker-compose -f docker-compose.prod.yml ps" -ForegroundColor Yellow
Write-Host "Container updaten: docker-compose -f docker-compose.prod.yml pull && docker-compose -f docker-compose.prod.yml up -d" -ForegroundColor Yellow
Write-Host ""
...
Write-Host "3. Backup-Strategie implementieren" -ForegroundColor Cyan
Write-Host "4. Monitoring-Dashboards konfigurieren" -ForegroundColor Cyan
Write-Host "5. Logs überwachen" -ForegroundColor Cyan
Write-Host ""
Write-Success "Der Server ist bereit für die HD App Production!"
