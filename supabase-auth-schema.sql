-- Human Design App - Supabase Auth Schema
-- Erweiterte Benutzer-Tabelle für Supabase Auth Integration

-- Users Table - Angepasst für Supabase Auth
CREATE TABLE IF NOT EXISTS users (
  ...
  subscription VARCHAR(20) DEFAULT 'free' CHECK (subscription IN ('free', 'basic', 'premium', 'vip')),
  hd_type VARCHAR(50),
  profile VARCHAR(20),
  centers TEXT[],
  channels TEXT[],
...
CREATE INDEX IF NOT EXISTS idx_readings_chart_id ON readings(chart_id);
...
CREATE INDEX IF NOT EXISTS idx_moon_tracking_date ON moon_tracking(date);
