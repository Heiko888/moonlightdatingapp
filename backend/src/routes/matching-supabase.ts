    ...
    const matches = await Promise.all(
      mockUsers
        ...
        .map(async (user) => {
          ...
          const mutualInterests = currentUser.interests.filter(interest => 
...
router.post('/analyze', authenticateToken, async (req, res) => {
  try {
    ...
    const compatibilityResult = await openaiService.analyzeCompatibility(user1, user2);
...
function calculateTypeCompatibility(type1: string, type2: string): number {
  ...
  return (compatibilityMatrix as any)[type1]?.[type2] || 50;
}

function calculateProfileCompatibility(profile1: string, profile2: string): number {
