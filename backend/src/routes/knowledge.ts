...
const knowledgeDbPath = path.join(__dirname, '../data/knowledge.json');

// Stelle sicher, dass das data-Verzeichnis existiert
const dataDir = path.dirname(knowledgeDbPath);
if (!fs.existsSync(dataDir)) {
  fs.mkdirSync(dataDir, { recursive: true });
}

// Lade Knowledge-Entries aus der Datei
const loadKnowledgeEntries = (): any[] => {
  try {
    if (fs.existsSync(knowledgeDbPath)) {
      const data = fs.readFileSync(knowledgeDbPath, 'utf8');
...
const saveKnowledgeEntries = (entries: any[]): void => {
  try {
    fs.writeFileSync(knowledgeDbPath, JSON.stringify(entries, null, 2));
  } catch (error) {
...
const initializeKnowledgeData = (): void => {
