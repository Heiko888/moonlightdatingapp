...
async function performComprehensiveAnalysis(chart1: any, chart2: any) {
  ...
  const relationshipProbability = calculateRelationshipProbability(
    compatibilityScore,
    energeticDynamics,
    centerInteractions
  );
...
function calculateBaseCompatibility(chart1: any, chart2: any): number {
  let score = 0;
...
function calculateEnergeticDynamics(chart1: any, chart2: any) {
...
function calculateAttraction(chart1: any, chart2: any): number {
...
// Harmonie berechnen
function calculateHarmony(chart1: any, chart2: any): number {
  let harmony = 50;
...
function calculateChallenge(chart1: any, chart2: any): number {
...
function calculateGrowth(chart1: any, chart2: any): number {
  let growth = 40;
...
function analyzeCenterInteractions(chart1: any, chart2: any) {
...
function analyzeChannelResonance(chart1: any, chart2: any) {
...
function analyzeGateInteractions(chart1: any, chart2: any) {
...
function calculateRelationshipProbability(
...
// Hilfsfunktionen
function getTypeCompatibility(type1: string, type2: string): number {
...
function getProfileCompatibility(profile1: string, profile2: string): number {
...
function getAuthorityCompatibility(auth1: string, auth2: string): number {
...
function getCenterCompatibility(centers1: any, centers2: any): number {
...
function getChannelCompatibility(channels1: any, channels2: any): number {
...
function getComplementaryCenters(centers1: any, centers2: any): string[] {
  const centers1List = Object.keys(centers1 || {}).filter(key => centers1[key]);
  const centers2List = Object.keys(centers2 || {}).filter(key => centers2[key]);
...
function getConflictingCenters(centers1: any, centers2: any): string[] {
...
function getProfileAttraction(profile1: string, profile2: string): number {
  const p1 = parseInt(profile1.split('/')[0]);
  const p2 = parseInt(profile2.split('/')[0]);
...
function getStrategyHarmony(strategy1: string, strategy2: string): number {
...
function getSharedChannels(channels1: any, channels2: any): string[] {
  const channels1List = Object.keys(channels1 || {}).filter(key => channels1[key]);
  const channels2List = Object.keys(channels2 || {}).filter(key => channels2[key]);
...
function getComplementaryChannels(channels1: any, channels2: any): string[] {
  // Erweiterte komplementäre Kanal-Erkennung
  return [];
}

function getConflictingChannels(channels1: any, channels2: any): string[] {
  // Erweiterte Konflikt-Kanal-Erkennung
  return [];
}

function getGrowthChannels(channels1: any, channels2: any): string[] {
  // Kanäle die Wachstum fördern
  return [];
}

function getHarmoniousGates(gates1: string[], gates2: string[]): string[] {
  // Harmonische Tor-Kombinationen
  return [];
}

function getChallengingGates(gates1: string[], gates2: string[]): string[] {
  // Herausfordernde Tor-Kombinationen
  return [];
}

function getMoonTransitInfluence(planets1: any, planets2: any): number {
...
function calculateConfidence(analysis: any): number {
  ...
  if (analysis.energeticDynamics.attraction > 0) confidence += 0.1;
  if (analysis.centerInteractions.complementary.length > 0) confidence += 0.1;
  if (analysis.channelResonance.shared.length > 0) confidence += 0.1;
...
async function startLiveAnalysis(connectionId: string, chart1: any, chart2: any) {
...
function stopLiveAnalysis(connectionId: string) {
