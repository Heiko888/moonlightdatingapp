...
import { AdvancedCompatibilityService } from '../services/advancedCompatibilityService';
...
router.post('/analyze', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const analysis = await AdvancedCompatibilityService.analyzeCompatibility(user1Id, user2Id);
    
    // Zusätzliche Beziehungs-spezifische Analyse
    const relationshipAnalysis = await analyzeRelationshipCompatibility(
      user1Id, 
      user2Id, 
      relationshipType
    );
 ...
 */
router.get('/compatible-users/:userId', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const compatibleUsers = await findCompatibleUsers({
      userId,
      minCompatibility: parseInt(minCompatibility as string),
 ...
 */
router.post('/batch-analyze', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const analyses = await Promise.all(
      targetIds.map(async (targetId: string) => {
        try {
          const analysis = await AdvancedCompatibilityService.analyzeCompatibility(userId, targetId);
          return {
            targetId,
            success: true,
            analysis
 ...
 */
router.get('/insights/:userId', authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = req.params.userId;
 ...
 */
async function analyzeRelationshipCompatibility(
  user1Id: string, 
  user2Id: string, 
...
async function findCompatibleUsers(filters: {
  ...
  try {
    ...
    const allUsers = localDb.db.prepare('SELECT * FROM users WHERE id != ?').all(filters.userId) as any[];
    ...
    for (const user of allUsers) {
      try {
        ...
        const analysis = await AdvancedCompatibilityService.analyzeCompatibility(filters.userId, user.id);
 ...
 * Persönliche Insights generieren
 */
async function generatePersonalInsights(userId: string) {
  return {
    profile: {
      hdType: 'Generator',
      profile: '5/1',
