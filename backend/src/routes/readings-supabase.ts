...
async function generateReadingWithAI(chartData: any, scope: string) {
  try {
...
async function generateReadingWithPDFs(chartData: any, scope: string) {
  try {
...
async function generateReflectionQuestionsWithPDFs(chartData: any, scope: string) {
  try {
...
function generateSimpleReading(chartData: any, scope: string) {
...
function saveReadings(readings: any[]) {
  try {
...
router.post('/pdf-enhanced', authenticateToken, async (req, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: 'Nicht authentifiziert' });
