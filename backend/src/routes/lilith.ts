...
router.get('/info', (req, res) => {
  try {
    const lilithInfo = getLilithInfo();
    if (!lilithInfo) {
...
router.get('/centers', (req, res) => {
  try {
    const centers = getAllLilithCenters();
    res.json(centers);
  } catch (error) {
...
router.get('/centers/:centerName', (req, res) => {
  try {
    ...
    const center = getLilithCenter(centerName.toUpperCase());
...
router.get('/gates/:gateNumber', (req, res) => {
  try {
    ...
    const gate = getLilithGate(gateNum);
...
router.get('/centers/:centerName/gates', (req, res) => {
  try {
    const { centerName } = req.params;
    const gates = getLilithGatesByCenter(centerName.toUpperCase());
