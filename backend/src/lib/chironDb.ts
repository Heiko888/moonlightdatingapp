...
const chironDbPath = path.join(process.cwd(), 'data', 'chiron.db');
const chironDb = new Database(chironDbPath);
...
function getAllChironTexts() {
  try {
    ...
    const chironTexts: { [key: number]: string } = {};
...
export function initChironDatabase() {
  ...
  Object.entries(allChironTexts).forEach(([gateNumber, deepMeaning]) => {
    ...
    const unconsciousMeaning = (unconsciousTexts as any)[gateNum] || '';
    ...
    insertGate.run(
      ...
      unconsciousMeaning,
      description,
      center?.center || 'UNKNOWN',
      center?.healing || 'Heilung',
      center?.wound || 'Wunde'
    );
  });
