...
export async function seedTestData() {
  ...
  try {
    ...
    for (const userData of testUsers) {
      ...
      const result = stmt.run(id, userData.username, userData.email, userData.password_hash, 
    ...
    const users = localDb.db?.prepare('SELECT id FROM users WHERE email LIKE \'%@example.com\'').all() as any[];
