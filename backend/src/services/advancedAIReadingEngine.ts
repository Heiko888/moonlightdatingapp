...
interface AdvancedReading {
  id: string;
  userId: string;
  type: 'personal' | 'relationship' | 'career' | 'spiritual' | 'health';
  content: {
    overview: string;
    typeAnalysis: string;
    authority: string;
...
export class AdvancedAIReadingEngine {
  ...
  async generateAdvancedReading(
    ...
    readingType: 'personal' | 'relationship' | 'career' | 'spiritual' | 'health',
    context?: string
  ): Promise<AdvancedReading> {
    try {
      ...
      const advancedReading: AdvancedReading = {
   ...
   */
  private async validateChartData(chart: ChartData): Promise<ValidationResult> {
    const errors: string[] = [];
    ...
    // Zentren-Validierung
    if (chart.centers) {
      const invalidCenters = chart.centers.filter(center => 
        !this.validationRules.centers.includes(center.name)
      );
    ...
    // Kanäle-Validierung
    if (chart.channels) {
      const invalidChannels = chart.channels.filter(channel => 
        !this.validationRules.channels.includes(channel.name)
      );
    ...
    // Gates-Validierung
    if (chart.gates) {
      const invalidGates = chart.gates.filter(gate => 
        !this.validationRules.gates.includes(gate.number)
      );
   ...
   */
  private async analyzeUserJourney(userProfile: UserProfile): Promise<any> {
...
Antworte in strukturiertem JSON-Format.`;
   ...
   */
  private async identifyPatterns(userProfile: UserProfile): Promise<any> {
...
Antworte in strukturiertem JSON-Format.`;
   ...
   */
  private async generateContextualReading(context: any): Promise<any> {
...
Antworte als JSON-Array mit den Fragen.`;
   ...
   */
  private parseReadingContent(content: string): any {
    const sections = {
      overview: '',
      typeAnalysis: '',
      authority: '',
...
export default new AdvancedAIReadingEngine();
