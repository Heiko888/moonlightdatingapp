...
export interface PlanetPosition {
  longitude: number;  // Ekliptische Länge in Grad
  latitude: number;   // Ekliptische Breite in Grad
  distance: number;   // Entfernung in AU
  speed: number;      // Geschwindigkeit in Grad/Tag
}
...
export class SwissEphemerisAlternative {
   ...
   */
  private calculateSunPosition(jd: number): PlanetPosition {
   ...
   */
  private calculateMoonPosition(jd: number): PlanetPosition {
    const T = (jd - 2451545.0) / 36525.0;
   ...
   */
  private calculatePlanetPosition(planet: string, jd: number): PlanetPosition {
    const T = (jd - 2451545.0) / 36525.0;
   ...
   */
  private getOrbitalElements(planet: string, T: number): any {
   ...
   */
  public calculatePositions(date: Date): EphemerisResult {
   ...
   */
  private calculateLilithPosition(jd: number): PlanetPosition {
   ...
   */
  public longitudeToGate(longitude: number): { gate: number; line: number } {
...
export const swissEphemerisAlternative = new SwissEphemerisAlternative();
