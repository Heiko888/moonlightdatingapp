...
export interface GeocodingConfig {
  provider: 'nominatim' | 'google' | 'mapbox' | 'fallback';
  apiKey?: string;
...
export class GeocodingService {
  ...
  constructor(config: GeocodingConfig = { provider: 'nominatim' }) {
    this.config = {
   ...
   */
  async geocodeBirthPlace(birthPlace: string): Promise<GeocodingResult> {
    try {
      // Cache prüfen
   ...
   */
  private async geocodeWithNominatim(birthPlace: string): Promise<GeocodingResult> {
   ...
   */
  private async geocodeWithFallback(birthPlace: string): Promise<GeocodingResult> {
    // Bekannte Städte mit Koordinaten
    const knownCities: Record<string, { lat: number; lon: number; country: string; countryCode: string; timezone: string }> = {
   ...
   */
  private getFallbackCoordinates(birthPlace: string): GeocodingResult {
   ...
   */
  private calculateConfidence(result: any, searchQuery: string): number {
   ...
   */
  private async getTimezoneFromCoordinates(lat: number, lon: number): Promise<string> {
    try {
