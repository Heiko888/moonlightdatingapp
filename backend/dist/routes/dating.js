...
const profilesFile = path_1.default.join(__dirname, '../../data/dating-profiles.json');
const matchesFile = path_1.default.join(__dirname, '../../data/matches.json');
const likesFile = path_1.default.join(__dirname, '../../data/likes.json');
const messagesFile = path_1.default.join(__dirname, '../../data/dating-messages.json');
...
const writeDatingProfiles = (profiles) => {
    try {
...
const readMatches = () => {
    try {
        if (fs_1.default.existsSync(matchesFile)) {
            const data = fs_1.default.readFileSync(matchesFile, 'utf8');
            return JSON.parse(data);
        }
    }
    catch (error) {
...
const writeMatches = (matches) => {
    try {
        fs_1.default.writeFileSync(matchesFile, JSON.stringify(matches, null, 2));
    }
    catch (error) {
...
const readLikes = () => {
    try {
        if (fs_1.default.existsSync(likesFile)) {
            const data = fs_1.default.readFileSync(likesFile, 'utf8');
            return JSON.parse(data);
        }
    }
    catch (error) {
...
const writeLikes = (likes) => {
    try {
        fs_1.default.writeFileSync(likesFile, JSON.stringify(likes, null, 2));
    }
    catch (error) {
...
const readDatingMessages = () => {
    try {
        if (fs_1.default.existsSync(messagesFile)) {
            const data = fs_1.default.readFileSync(messagesFile, 'utf8');
            return JSON.parse(data);
        }
    }
    catch (error) {
...
const writeDatingMessages = (messages) => {
    try {
        fs_1.default.writeFileSync(messagesFile, JSON.stringify(messages, null, 2));
    }
    catch (error) {
...
class HumanDesignMatcher {
    // Typ-Kompatibilität
    static getTypeCompatibility(type1, type2) {
    ...
    static getProfileCompatibility(profile1, profile2) {
    ...
    static getAuthorityCompatibility(authority1, authority2) {
    ...
    static getCentersCompatibility(centers1, centers2) {
    ...
    static getChannelsCompatibility(channels1, channels2) {
    ...
    // Gate-Kompatibilität
    static getGatesCompatibility(gates1, gates2) {
    ...
    static getIncarnationCrossCompatibility(cross1, cross2) {
    ...
    static calculateCompatibility(profile1, profile2) {
        ...
        const totalCompatibility = typeCompat * weights.type +
...
router.get('/profiles', (req, res) => {
    try {
        ...
        const sortedProfiles = filteredProfiles.sort((a, b) => new Date(b.lastActive).getTime() - new Date(a.lastActive).getTime());
...
router.get('/matches/:userId', (req, res) => {
    try {
        ...
        const userMatches = matches.filter(m => (m.user1Id === userId || m.user2Id === userId) && m.isActive);
...
router.get('/recommendations/:userId', (req, res) => {
    try {
        ...
        const availableProfiles = profiles.filter(p => p.userId !== userId &&
            p.isActive &&
