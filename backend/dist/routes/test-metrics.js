    ...
    try {
        ...
        switch (randomActivity) {
            case 'userRegistration':
                grafana_cloud_1.recordAppMetrics.userRegistration();
                (0, grafana_cloud_1.logToGrafana)('info', 'Test user registration', {
                    testRun: true,
                    timestamp: new Date().toISOString()
                });
                break;
            case 'chartGenerated':
                grafana_cloud_1.recordAppMetrics.chartGenerated();
                (0, grafana_cloud_1.logToGrafana)('info', 'Test chart generation', {
                    testRun: true,
                    chartType: 'generator',
                    timestamp: new Date().toISOString()
                });
                break;
            case 'matchCreated':
                grafana_cloud_1.recordAppMetrics.matchCreated();
                (0, grafana_cloud_1.logToGrafana)('info', 'Test match creation', {
                    testRun: true,
            ...
            case 'moonPhaseCalculated':
                grafana_cloud_1.recordAppMetrics.moonPhaseCalculated();
                (0, grafana_cloud_1.logToGrafana)('info', 'Test moon phase calculation', {
                    testRun: true,
                    phase: 'fullmoon',
                    timestamp: new Date().toISOString()
                });
            ...
            case 'datingImpulseGenerated':
                grafana_cloud_1.recordAppMetrics.datingImpulseGenerated();
                (0, grafana_cloud_1.logToGrafana)('info', 'Test dating impulse generation', {
                    testRun: true,
                    impulseType: 'energy',
                    timestamp: new Date().toISOString()
                });
            ...
            case 'liveEventJoined':
                grafana_cloud_1.recordAppMetrics.liveEventJoined();
                (0, grafana_cloud_1.logToGrafana)('info', 'Test live event join', {
                    testRun: true,
                    eventType: 'workshop',
                    timestamp: new Date().toISOString()
                });
            ...
            case 'courseCompleted':
                grafana_cloud_1.recordAppMetrics.courseCompleted();
                (0, grafana_cloud_1.logToGrafana)('info', 'Test course completion', {
                    testRun: true,
                    courseType: 'video',
                    timestamp: new Date().toISOString()
                });
        ...
        (0, grafana_cloud_1.logToGrafana)('error', 'Failed to generate test metrics', {
            error: error.message,
            stack: error.stack,
            timestamp: new Date().toISOString()
        });
...
router.post('/generate-bulk-metrics', async (req, res) => {
    try {
        ...
        (0, grafana_cloud_1.logToGrafana)('info', 'Bulk test metrics generated', {
            count,
            testRun: true,
            timestamp: new Date().toISOString()
        });
        ...
        (0, grafana_cloud_1.logToGrafana)('error', 'Failed to generate bulk test metrics', {
            error: error.message,
            stack: error.stack,
            timestamp: new Date().toISOString()
        });
...
router.post('/generate-test-logs', async (req, res) => {
    try {
        ...
        (0, grafana_cloud_1.logToGrafana)(randomLevel, `Test: ${randomMessage}`, {
            testRun: true,
            level: randomLevel,
...
router.get('/status', async (req, res) => {
    try {
        ...
        (0, grafana_cloud_1.logToGrafana)('info', 'Grafana Cloud status checked', {
            status,
            timestamp: new Date().toISOString()
        });
        res.json(status);
