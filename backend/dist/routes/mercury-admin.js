...
router.get('/gate/:gateNumber', async (req, res) => {
    try {
        ...
        const gate = (0, localDb_1.getMercuryGate)(gateNumber);
        if (!gate) {
...
router.put('/gate/:gateNumber', async (req, res) => {
    try {
        ...
        const result = (0, localDb_1.updateMercuryGate)(gateNumber, filteredData);
        if (result.success) {
            // Aktualisierte Daten zurückgeben
            const updatedGate = (0, localDb_1.getMercuryGate)(gateNumber);
            res.json({
                success: true,
...
router.get('/gates', async (req, res) => {
    try {
        const gates = [];
        for (let i = 1; i <= 64; i++) {
            const gate = (0, localDb_1.getMercuryGate)(i);
            if (gate) {
                gates.push({
