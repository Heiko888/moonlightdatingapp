...
const bookingsFile = path_1.default.join(__dirname, '../../data/bookings.json');
const availabilityFile = path_1.default.join(__dirname, '../../data/availability.json');
...
const readBookings = () => {
    try {
        if (fs_1.default.existsSync(bookingsFile)) {
            const data = fs_1.default.readFileSync(bookingsFile, 'utf8');
            return JSON.parse(data);
        }
    }
    catch (error) {
...
const writeBookings = (bookings) => {
    try {
        fs_1.default.writeFileSync(bookingsFile, JSON.stringify(bookings, null, 2));
    }
    catch (error) {
...
const readAvailability = () => {
    try {
        if (fs_1.default.existsSync(availabilityFile)) {
            const data = fs_1.default.readFileSync(availabilityFile, 'utf8');
            return JSON.parse(data);
        }
    }
    catch (error) {
...
const writeAvailability = (availability) => {
    try {
        fs_1.default.writeFileSync(availabilityFile, JSON.stringify(availability, null, 2));
    }
    catch (error) {
...
router.get('/availability/:coachId', (req, res) => {
    try {
...
router.get('/available-slots/:coachId/:date', (req, res) => {
    try {
        ...
        const dayAvailability = availability.find(a => a.coachId === coachId && a.dayOfWeek === dayOfWeek && a.isAvailable);
        if (!dayAvailability) {
            return res.json({
                success: true,
                slots: []
        ...
        for (let time = new Date(startTime); time < endTime; time.setMinutes(time.getMinutes() + 60)) {
            ...
            const isBooked = bookings.some(booking => booking.coachId === coachId &&
                booking.date === date &&
