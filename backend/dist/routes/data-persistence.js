...
router.post('/user-profile', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        const { name, birthdate, birthplace, hd_type, profile, centers, channels, gates, planets, chart_data, avatar } = req.body;
        if (!userId) {
      ...
      WHERE id = ?
    `);
        const result = updateStmt.run(name, birthdate, birthplace, hd_type, profile, JSON.stringify(centers), JSON.stringify(channels), JSON.stringify(gates), JSON.stringify(planets), JSON.stringify(chart_data), avatar, userId);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'Benutzer nicht gefunden' });
        }
        // Metriken aktualisieren
        if (hd_type) {
            prometheus_1.hdTypeDistribution.labels(hd_type).inc();
        }
        if (profile) {
            prometheus_1.profileDistribution.labels(profile).inc();
        }
        prometheus_1.databaseOperations.labels('update', 'users').inc();
        res.json({
            success: true,
...
router.post('/chart', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        const { name, birth_date, birth_time, birth_place, chart_data, centers, channels, gates, planets } = req.body;
        if (!userId) {
            return res.status(401).json({ error: 'Benutzer nicht authentifiziert' });
        }
        ...
        if (chart_data?.hdType) {
            prometheus_1.hdTypeDistribution.labels(chart_data.hdType).inc();
        }
        if (chart_data?.profile) {
            prometheus_1.profileDistribution.labels(chart_data.profile).inc();
        }
...
router.get('/charts', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Benutzer nicht authentifiziert' });
...
router.post('/moon-tracking', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        const { date, moon_phase, mood, energy, notes, rituals_completed, journal_entry_id } = req.body;
        if (!userId) {
            return res.status(401).json({ error: 'Benutzer nicht authentifiziert' });
        }
        if (!date || !moon_phase) {
    ...
    `);
        ...
        const result = stmt.run(id, userId, date, moon_phase, mood, energy, notes, JSON.stringify(rituals_completed), journal_entry_id);
        prometheus_1.databaseOperations.labels('insert', 'moon_tracking').inc();
        res.status(201).json({
            success: true,
            trackingId: id,
...
router.get('/moon-tracking', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
...
router.post('/journal', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
    ...
    `);
        const id = Math.random().toString(36).substr(2, 9);
        const result = stmt.run(id, userId, title, content, mood, energy_level, JSON.stringify(tags));
        prometheus_1.databaseOperations.labels('insert', 'journal_entries').inc();
        res.status(201).json({
            success: true,
            entryId: id,
...
router.get('/journal', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
      ...
      LIMIT ? OFFSET ?
    `);
        const entries = stmt.all(userId, parseInt(limit), parseInt(offset));
        // Parse JSON-Felder
        const parsedEntries = entries.map(entry => ({
            ...entry,
...
function calculateUserAnalytics(moonEntries, journalEntries, charts) {
    const analytics = {
        // Grundstatistiken
        totalMoonEntries: moonEntries.length,
        totalJournalEntries: journalEntries.length,
