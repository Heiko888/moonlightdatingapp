...
const generateBioFromUser = (user) => {
    const parts = [];
    if (user.hd_type) {
...
const getStrategyByType = (hdType) => {
    const strategies = {
...
const getAuthorityByType = (hdType) => {
    const authorities = {
        'Generator': 'Sakral',
...
router.get('/profiles/:userId', async (req, res) => {
    try {
        ...
        let filteredProfiles = allUsers.filter((user) => user.id !== userId);
        // Konvertiere zu Swipe-Format
        const swipeProfiles = filteredProfiles.map((user) => {
            ...
            const interests = [];
            if (user.hd_type)
                interests.push(user.hd_type);
            if (user.profile)
                interests.push(`Profil ${user.profile}`);
            if (user.subscriptionPlan)
                interests.push(`${user.subscriptionPlan} Member`);
        ...
        if (hdType && hdType !== '') {
            filteredProfiles = swipeProfiles.filter((profile) => profile.hdType === hdType);
        }
        else {
        ...
        // Filtere nach Stadt
        if (city && city !== '') {
            filteredProfiles = filteredProfiles.filter((profile) => profile.city.toLowerCase().includes(city.toString().toLowerCase()));
        }
        // Filtere nach Interessen
        if (interests && interests !== '') {
            const interestArray = interests.toString().split(',').map(i => i.trim());
            filteredProfiles = filteredProfiles.filter((profile) => profile.interests.some((interest) => interestArray.some(searchInterest => interest.toLowerCase().includes(searchInterest.toLowerCase()))));
        }
    ...
    `);
        ...
        if (liked) {
            ...
            if (userProfile && targetProfile) {
                // Parse JSON-Felder
                const userCenters = userProfile.centers ? JSON.parse(userProfile.centers) : {};
                const userChannels = userProfile.channels ? JSON.parse(userProfile.channels) : {};
                const targetCenters = targetProfile.centers ? JSON.parse(targetProfile.centers) : {};
                const targetChannels = targetProfile.channels ? JSON.parse(targetProfile.channels) : {};
                // Berechne Kompatibilität
                compatibilityScore = calculateCompatibilityScore({ ...userProfile, centers: userCenters, channels: userChannels }, { ...targetProfile, centers: targetCenters, channels: targetChannels });
...
router.get('/matches/:userId', async (req, res) => {
    ...
    `).all(userId, userId);
        // Konvertiere zu Swipe-Format
        const swipeMatches = matches.map((match) => ({
            id: match.id,
            userA: {
                _id: match.user_a,
...
router.get('/history/:userId', async (req, res) => {
    ...
    `).all(userId);
        // Konvertiere zu Swipe-Format
        const swipeHistory = swipes.map((swipe) => ({
            id: swipe.id,
...
router.get('/new-matches/:userId/:since', async (req, res) => {
    try {
        const userId = req.params.userId;
      ...
      ORDER BY m.created_at DESC
    `).all(userId, userId, since.toISOString());
        // Konvertiere zu Swipe-Format
        const swipeNewMatches = newMatches.map((match) => ({
            id: match.id,
            userA: {
                _id: match.user_a,
                name: match.user_a_name,
...
function generateBio(hdType, profile, centers, channels) {
