...
// NASA API Funktionen
async function getNASAEphemeris(date) {
    try {
        const response = await axios_1.default.get(`${NASA_BASE_URL}/ephemeris`, {
            params: {
                target: 'moon',
                date: date,
...
async function calculateMoonPhaseWithNASA(date) {
...
function mapNASAPhaseToLocal(nasaPhase, illumination) {
...
function calculateMoonPhaseLocal(date) {
...
router.get('/phases/:date', async (req, res) => {
    try {
...
router.get('/phases', auth_1.authenticateToken, async (req, res) => {
    try {
        res.json({
            phases: moonPhases,
    ...
    try {
        ...
        if (supabase_1.supabase) {
            try {
                ...
                const { data: journalData, error: journalError } = await supabase_1.supabase
...
router.get('/tracking/:user_id/stats', async (req, res) => {
    try {
        const { user_id } = req.params;
        // Versuche Supabase zu verwenden
        if (supabase_1.supabase) {
...
router.get('/human-design/:hd_type', async (req, res) => {
    try {
        ...
        const recommendations = hdRecommendations[hd_type] || {
...
function calculateTrackingStats(trackingData) {
    ...
    const mostCommonPhase = Object.keys(phaseCounts).reduce((a, b) => phaseCounts[a] > phaseCounts[b] ? a : b);
...
router.get('/stories', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (moon_phase) {
            filteredStories = filteredStories.filter(story => story.moon_phase.toLowerCase() === moon_phase.toString().toLowerCase());
        }
        if (culture) {
            filteredStories = filteredStories.filter(story => story.culture.toLowerCase().includes(culture.toString().toLowerCase()));
        }
        res.json({
...
router.get('/plant-rituals', async (req, res) => {
    try {
        const { moon_phase, plant_type } = req.query;
        let filteredRituals = plantRituals;
        if (moon_phase) {
            filteredRituals = filteredRituals.filter(ritual => ritual.moon_phase.toLowerCase() === moon_phase.toString().toLowerCase());
        }
        if (plant_type) {
            filteredRituals = filteredRituals.filter(ritual => ritual.plants.some(plant => plant.toLowerCase().includes(plant_type.toString().toLowerCase())));
        }
        res.json({
...
router.get('/health-guidance', async (req, res) => {
    try {
        const { moon_phase } = req.query;
        let filteredGuidance = healthGuidance;
        if (moon_phase) {
            filteredGuidance = filteredGuidance.filter(guidance => guidance.moon_phase.toLowerCase() === moon_phase.toString().toLowerCase());
        }
        res.json({
    ...
    try {
        ...
        for (let month = 0; month < 12; month++) {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                const dayStories = moonStories.filter(story => story.moon_phase === moonPhase.name);
                const dayRituals = plantRituals.filter(ritual => ritual.moon_phase === moonPhase.name);
                const dayHealth = healthGuidance.find(guidance => guidance.moon_phase === moonPhase.name);
                yearlyCalendar.push({
                    date: date.toISOString().split('T')[0],
                    day: day,
                    month: month + 1,
...
router.get('/monthly/:year/:month', auth_1.authenticateToken, async (req, res) => {
    try {
        const { year, month } = req.params;
        const yearNum = parseInt(year);
...
// Hilfsfunktionen
function isSpecialMoonDay(date, moonPhase) {
    ...
    return specialPhases.includes(moonPhase) ||
        specialDates.some(special => Math.abs(month - special.month) <= 1 && Math.abs(day - special.day) <= 3);
}
function getPhaseDistribution(calendar) {
    const distribution = {};
...
router.get('/gate-details', async (req, res) => {
    try {
        ...
        if (center) {
            filteredGates = filteredGates.filter(gate => gate.center.toLowerCase().includes(center.toString().toLowerCase()));
        }
        res.json({
            gates: filteredGates,
...
router.get('/gate-details/:id', async (req, res) => {
    try {
        const { id } = req.params;
