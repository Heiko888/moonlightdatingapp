...
router.get('/symbol/:symbolName', (req, res) => {
    try {
        ...
        const symbol = reikiSymbols[symbolName];
        if (!symbol) {
...
router.post('/session-plan', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.post('/self-treatment', auth_1.authenticateToken, async (req, res) => {
    try {
