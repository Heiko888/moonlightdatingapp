...
exports.logToGrafana = exports.recordAppMetrics = exports.grafanaCloudMiddleware = exports.sendTracesToGrafana = exports.sendLogsToGrafana = exports.sendMetricsToGrafana = void 0;
...
const sendMetricsToGrafana = async (metrics) => {
    ...
    try {
        const response = await fetch(GRAFANA_CLOUD_CONFIG.prometheus.remoteWriteUrl, {
            ...
            headers: {
                ...
                'Authorization': `Basic ${Buffer.from(`${GRAFANA_CLOUD_CONFIG.prometheus.username}:${GRAFANA_CLOUD_CONFIG.prometheus.password}`).toString('base64')}`
            },
            body: metrics
        });
        if (!response.ok) {
...
exports.sendMetricsToGrafana = sendMetricsToGrafana;
// Loki Log Shipping
const sendLogsToGrafana = async (logs) => {
    // Skip if credentials are not configured or are placeholder values
    ...
    try {
        ...
        const response = await fetch(GRAFANA_CLOUD_CONFIG.loki.pushUrl, {
            ...
            headers: {
                ...
                'Authorization': `Basic ${Buffer.from(`${GRAFANA_CLOUD_CONFIG.loki.username}:${GRAFANA_CLOUD_CONFIG.loki.password}`).toString('base64')}`
            },
...
exports.sendLogsToGrafana = sendLogsToGrafana;
// Tempo Tracing
const sendTracesToGrafana = async (traces) => {
    try {
        const response = await fetch(GRAFANA_CLOUD_CONFIG.tempo.otlpEndpoint, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-protobuf',
                'Authorization': `Basic ${Buffer.from(`${GRAFANA_CLOUD_CONFIG.tempo.username}:${GRAFANA_CLOUD_CONFIG.tempo.password}`).toString('base64')}`
            },
...
exports.sendTracesToGrafana = sendTracesToGrafana;
// Enhanced Prometheus Middleware with Grafana Cloud Integration
const grafanaCloudMiddleware = (req, res, next) => {
    const start = Date.now();
    // Original prometheus middleware
    (0, prometheus_1.prometheusMiddleware)(req, res, next);
    ...
    res.on('finish', async () => {
        ...
        await (0, exports.sendMetricsToGrafana)(metrics);
    });
};
exports.grafanaCloudMiddleware = grafanaCloudMiddleware;
// Application Metrics for Human Design App
exports.recordAppMetrics = {
    userRegistration: () => {
        const metric = 'hd_app_users_total 1';
        (0, exports.sendMetricsToGrafana)(metric);
    },
    chartGenerated: () => {
...
// Logging Helper
const logToGrafana = (level, message, context) => {
    const logEntry = {
    ...
    (0, exports.sendLogsToGrafana)([logEntry]);
    console.log(`[${level.toUpperCase()}] ${message}`, context);
};
exports.logToGrafana = logToGrafana;
exports.default = {
    sendMetricsToGrafana: exports.sendMetricsToGrafana,
    sendLogsToGrafana: exports.sendLogsToGrafana,
    sendTracesToGrafana: exports.sendTracesToGrafana,
    grafanaCloudMiddleware: exports.grafanaCloudMiddleware,
    recordAppMetrics: exports.recordAppMetrics,
    logToGrafana: exports.logToGrafana
};
