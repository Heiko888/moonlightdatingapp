...
async function testTestDataSeeding() {
    ...
    try {
        const response = await (0, node_fetch_1.default)(`${BASE_URL}/test-data/seed`, {
            method: 'POST',
            headers: {
...
async function testTestDataStatus() {
    ...
    try {
        const response = await (0, node_fetch_1.default)(`${BASE_URL}/test-data/status`);
        const result = await response.json();
        if (result.success) {
...
async function testTestDataUsers() {
    ...
    try {
        const response = await (0, node_fetch_1.default)(`${BASE_URL}/test-data/users`);
        const result = await response.json();
        if (result.success) {
...
async function testSwipeProfiles() {
    ...
    try {
        ...
        const usersResponse = await (0, node_fetch_1.default)(`${BASE_URL}/test-data/users`);
        ...
        const response = await (0, node_fetch_1.default)(`${BASE_URL}/api/swipe/profiles/${userId}`);
        const result = await response.json();
        if (Array.isArray(result)) {
            console.log('✅ Swipe-Profile API erfolgreich');
            console.log(`📊 ${result.length} Profile für Swipe verfügbar`);
            result.slice(0, 3).forEach((profile, index) => {
...
async function testAdvancedMatching() {
    ...
    try {
        ...
        const response = await (0, node_fetch_1.default)(`${BASE_URL}/api/advanced-matching/analyze`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function testCompatibleUsers() {
    ...
    try {
        ...
        const userId = usersResult.data.users[0].id;
        const response = await (0, node_fetch_1.default)(`${BASE_URL}/api/advanced-matching/compatible-users/${userId}?minCompatibility=70&limit=5`);
        const result = await response.json();
        if (result.success) {
