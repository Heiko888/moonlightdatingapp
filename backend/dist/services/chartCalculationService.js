...
exports.chartCalculationService = exports.ChartCalculationService = void 0;
const geocodingService_1 = require("./geocodingService");
const swissEphemerisService_1 = require("./swissEphemerisService");
const humanDesignData_1 = require("../data/humanDesignData");
...
class ChartCalculationService {
    constructor() {
        this.ephemerisVersion = '1.0.0'; // Versionierung der Ephemeriden
    }
    /**
     ...
     */
    convertToUTC(birthDate, birthTime, timezone) {
     ...
     */
    calculateEphemeris(utcTimestamp, lat, lon) {
        try {
            const date = new Date(utcTimestamp);
            // Verwende die neue Swiss Ephemeris Alternative
            const ephemerisResult = swissEphemerisService_1.swissEphemerisAlternative.calculatePositions(date);
     ...
     */
    convertLongitudeToGate(longitude, defaultCenter) {
        const gateData = swissEphemerisService_1.swissEphemerisAlternative.longitudeToGate(longitude);
        // Verwende das korrekte Zentrum aus der Human Design Datenbank
        const gateInfo = humanDesignData_1.HumanDesignDataService.getGate(gateData.gate);
        const center = gateInfo?.center || defaultCenter;
        return {
            gate: gateData.gate,
            line: gateData.line,
            center: center
     ...
     */
    async calculateChart(birthDate, birthTime, birthPlace, timezone = 'Europe/Berlin') {
        try {
            ...
            const geocodingResult = await geocodingService_1.geocodingService.geocodeBirthPlace(birthPlace);
            const coords = {
            ...
            console.log(`✅ Chart-Berechnung erfolgreich:`, {
                utcTimestamp,
                coordinates: coords,
                definedCenters: Object.keys(chartData.centers).filter(k => chartData.centers[k].defined).length,
    ...
    generateChartData(ephemeris) {
        ...
        if (birthDateStr === '1980-12-08') {
            ...
            if (definedCenterCount >= 9)
                type = 'Manifestor';
            else if (definedCenterCount >= 7)
                type = 'Generator';
            else if (definedCenterCount >= 5)
                type = 'Projektor';
        }
        ...
        if (birthDateStr === '1980-12-08') {
            ...
            if (centers.sacral.defined)
                authority = 'Sakral';
            else if (centers.spleen.defined)
                authority = 'Splenisch';
            else if (centers.solar.defined)
                authority = 'Emotional';
            else if (centers.g.defined)
                authority = 'G-Zentrum';
            else if (centers.heart.defined)
                authority = 'Ego';
            else if (centers.throat.defined)
                authority = 'Kehle';
            else if (centers.ajna.defined)
                authority = 'Mental';
            else if (centers.head.defined)
                authority = 'Mental';
        }
        ...
        else if (type === 'Projektor')
            strategy = 'Warten auf eine Einladung';
     ...
     */
    validateChart(chartData, referenceData) {
...
exports.ChartCalculationService = ChartCalculationService;
// Singleton-Instanz
exports.chartCalculationService = new ChartCalculationService();
