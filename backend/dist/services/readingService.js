...
class ReadingService {
    ...
    static getModulesByIds(moduleIds) {
        const allModules = this.getAllModules();
        return moduleIds.map(id => allModules.find(module => module.id === id)).filter(Boolean);
    }
    // Reading generieren
    ...
    // Reading-Inhalt generieren
    static async generateReadingContent(user, modules, context) {
        let content = '';
    ...
    // Platzhalter ersetzen
    static replacePlaceholders(content, userData, context) {
        let result = content;
    ...
    static generateDynamicContent(content, userData) {
        let result = content;
    ...
    static getDefinedCenters(centers) {
        if (!centers || typeof centers !== 'object')
            return 'Keine definierten Zentren';
    ...
    static getOpenCenters(centers) {
        if (!centers || typeof centers !== 'object')
            return 'Keine offenen Zentren';
        const open = Object.keys(centers).filter(key => centers[key] === false);
        return open.length > 0 ? open.join(', ') : 'Keine offenen Zentren';
    }
    static getDefinedChannels(channels) {
        if (!channels || typeof channels !== 'object')
            return 'Keine definierten Kanäle';
    ...
    static getDefinedGates(gates) {
        if (!gates || typeof gates !== 'object')
            return 'Keine definierten Tore';
